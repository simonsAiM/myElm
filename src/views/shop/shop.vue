          ...
          <router-link to="/shop/shopDetail" class="description_top">
            ...
            <section class="description_right">
              ...
              <p class="description_text">
                商家配送/{{ shopDetailData.order_lead_time }}分钟送达/配送费¥{{
                  shopDetailData.float_delivery_fee
                }}
              </p>
                ...
                <section class="rating_header_right">
                  <p>
                    ...
                    <span class="rating_num">{{
                      ratingScoresData.service_score.toFixed(2)
                    }}</span>
                  </p>
                  <p>
                    <span>菜品评价</span>
                    ...
                    ></rating-star>
                    <span class="rating_num">{{
                      ratingScoresData.food_score.toFixed(2)
                    }}</span>
                  </p>
                  <p>
                    <span>送达时间</span>
              ...
              <ul class="tag_list_ul">
                <li
                  ...
                  :class="{
                    ...
                    tagActivity: ratingTagIndex == index,
                  }"
...
export default {
  data() {
    return {
      ...
      categoryNum: [],  // 商品类型右上角以加入购物车的数量
      totalPrice: 0,  //总共价格
      cartFoodList: [], // 购物车商品列表
      showCartList: false,  // 显示购物车列表
      receiveInCart: false, // 购物车组件下落的圆点是否到达目标位置
      ratingList: null, // 评价列表
      ratingOffet: 0, // 评价获取数据offset值
      ratingScoresData: null, // 评价总体分数
      ratingTagsList: null, // 评价分类列表
      ratingTagIndex: 0,  // 评价分类索引
      preventRepeatRequest: false,  // 防止多次触发数据请求
      ratingTagName: '', // 评论的类型
      loadRatings: false, // 加载更多评论时显示鸡啊在组件
      foodScroll: null, // 食品列表scroll
      showSpecs: null,  // 控制显示食品规格
  ...
  methods: {
    ...
    async initData() {
      ...
      // 获取商铺信息
      let shopDetailData = await shopDetails(this.shopId, this.latitude, this.longitude)
      this.shopDetailData = JSON.parse(shopDetailData)

      // 获取商铺视频列表
      let menuList = await foodMenu(this.shopId)
      this.menuList = JSON.parse(menuList)
      // 评论列表
      let ratingList = await getRatingList(this.shopId, this.ratingOffet)
      this.ratingList = JSON.parse(ratingList)
      // 商铺评论详情
      let ratingScoresData = await ratingScores(this.shopId)
      this.ratingScoresData = JSON.parse(ratingScoresData)
      // 评论Tag列表
      let ratingTagsList = await ratingTags(this.shopId)
      this.ratingTagsList = JSON.parse(ratingTagsList)
      this.RECORD_SHOPDETAIL(this.shopDetailData)
      // 隐藏加载动画
      this.hideLoading()
    },
    // 获取视频列表的高度，存入shopListTop
    getFoodListHeight() {
    ...
    */
    ...
    async loaderMoreRating() {
      ...
      this.ratingOffet = 0
      let ratingDate = await getRatingList(this.shopId, this.ratingOffet, this.ratingTagName)
      let ratingDateJson = JSON.parse(ratingDate)
      this.ratingList = [...this.ratingList, ...ratingDateJson]
      this.loadRatings = false
      if (this.ratingList.length >= 10) {
    ...
    beforeEnter(el) {
      ...
      e.children[0].style.transform = `translate3d(${this.elLeft -30}px, 0, 0)`
    ...
    afterEnter(el) {
      ...
      el.children[0].addEventListener('tansitionend', () => {
        this.listenInCart()
      })
  ...
  computed: {
    ...
    deliveryFee: function () {
      if (this.shopDetailData) {
        return this.shopDetailData.float_deliery_fee
      } else {
        return null
      }
    },
