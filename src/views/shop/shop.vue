          ...
          >
            <p class="ellipsis">
              <span
                class="tip_icon"
                :style="{
                  backgroundColor: '#' + shopDetailData.activities[0].ico_color,
...
export default {
  ...
  methods: {
    ...
    listenScroll(element) {
      ...
      this.foodScroll.on('scroll', pos => {
        ...
        this.shopListTop.forEach((item, index) => {
          if (this.menuIndexChange && Math.abs(round(pos.y)) >= item) {
    ...
    chooseMenu(index) {
      ...
      this.foodScroll.scrollTo(0, -this.shopList[index], 400)
