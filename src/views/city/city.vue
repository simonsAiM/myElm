...
export default {
  data() {
    return {
      ...
      cityname: '', // 当前城市名称
      inputValue: '', // 搜索地址
      historytitle: true, // 默认显示搜索历史，点击搜索后隐藏
      placelist: [], // 搜索城市列表
      placeHistorty: [], // 搜索历史列表
      placeNone: '' // 搜索无结果，显示提示信息
    }
  },
  mounted() {
  ...
  methods: {
    ...
    postpois () { 
      if (this.inputValue) {
        searchplace(this.cityid, this.inputValue).then(res => {
          ...
          this.placelist = resJson
          this.placeNone = resJson.length ? false: true
        })
      }
    },
    /**
    ...
    nextpage(index, geohash) {
      ...
      if(history) {
        ...
        this.placeHistorty = JSON.parse(history)
        this.placeHistorty.forEach(item => {
