        ...
        methods: {
            ...
            async getVerifyCode(){
                if (this.rightPhoneNumber) {
                    ...
                    if (resJson.message) {
                        this.showAlert = true;
                        this.alertText = resJson.message;
                        return
                    }else if(!resJson.is_exists) {
                        this.showAlert = true;
                        this.alertText = '您输入的手机号尚未绑定';
                        return
                    }
                    //获取验证信息
                    let getCodeRes = await mobileCode(this.phoneNumber);
                    let getCode = JSON.parse(getCodeRes)
                    if (getCode.message) {
                        this.showAlert = true;
            ...
            async resetButton(){
                ...
                // 发送重置信息
                let changeRes = await changePassword(this.phoneNumber, this.oldPassWord, this.newPassWord, this.confirmPassWord, this.mobileCode);
                let res = JSON.parse(changeRes)
                if (res.message) {
                    this.showAlert = true;
