        ...
        <transition name="showlist">
          <section v-show="sortBy == 'activity'" class="sort_detail_type filter_container">
            <section style="width: 100%">
              ...
              <ul class="filter_ul">
                <li v-for="(item, index) in Delivery" :key="index" @click="selectDeliveryMode(item.id)" class="filter_li">
...
export default {
  ...
  methods: {
    ...
    async initData() {
      ...
      // 获取筛选列表的配送方式
      let deliery = await foodDelivery(this.latitude, this.longitude)
      this.Delivery = JSON.parse(deliery)
      let activity = await foodActivity(this.latitude, this.longitude)
      this.Activity = JSON.parse(activity)
      // 记录support_ids的状态，默认不选中，点击状态取反，status为true时选中状态
