    ...
    export default {
        ...
        methods: {
            ...
            async initData(){
                ...
                //检验订单是否满足条件
                let res = await checkout(this.geohash, [newArr], this.shopId);
                this.checkoutData = JSON.parse(res)
            ...
            async initAddress(){
                if (this.userInfo && this.userInfo.user_id) {
                    const res = await getAddressList(this.userInfo.user_id);
                    const addressRes = JSON.parse(res)
            ...
            async confrimOrder(){
                ...
                //发送订单信息
                let res = await placeOrders(this.userInfo.user_id, this.checkoutData.cart.id, this.choosedAddress.id, this.remarklist, this.checkoutData.cart.groups, this.geohash, this.checkoutData.sig);
                let orderRes = JSON.parse(res)
