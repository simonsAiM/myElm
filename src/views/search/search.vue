<template>
  <div class="paddingTop search_page">
    ...
    <form class="search_form">
      <input type="search" name="search" placeholder="请输入商家或没事名称" class="search_input" v-model="searchValue" @input="checkInput">
    ...
    <section v-if="restaurantList.length">
      ...
      <ul class="list_container">
        <router-link :to="{path: '/shop', query: {id: item.id}}" v-for="item in restaurantList" :key="item.id" class="list_li">
          <li>
            ...
            <section class="item_right">
              <div class="item_right_text">
                ...
                <p>{{item.delivery_fee || item.float_minium_order_amount}} 元起送/距离{{item.distance}}</p>
              </div>
  ...
  export default {
    ...
    mounted() {
      ...
      if (getStore('searchHistory')) {
        this.showHistory = JSON.parse(getStore('searchHistory'))
      }
    },
    methods: {
      async searchTarget(historyValue) {
        ...
        let restaurantList = await searchRestaurant(this.geohash, this.searchValue)
        this.restaurantList = JSON.parse(restaurantList)
      ...
      // 搜索结束后，删除搜索内容知道为空时清空搜索结果，并显示历史记录
      checkInput() {
        if (this.searchValue === '') {
