<template>
  <div class="order_page">
    ...
    <ul class="order_list_ul" v-load-more="loaderMore">
      <li class="order_list_js" v-for="item in orderList" :key="item.id">
        <img :src=" imgBaseUrl + item.restaurant_image_url" alt="" class="restaurant_image">
        <section class="order_item_right">
          ...
          <section class="order_basket">
            ...
            <p class="order_amount">¥{{item.total_amout.toFixed(2)}}</p>
          </section>
          <div class="order_again">
            <compute-time v-if="item.status_bar.title === '等待支付'" :time="item.time_pass"></compute-time>
            <router-link :to="{path: '/shop',query: {geohash, id: item.restaurant_id}}" class="buy_again" v-else>
              <span>再来一单</span>
            </router-link>
          </div>
        </section>
