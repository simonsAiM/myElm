...
export default {
  data() {
    return {
      offset: 0, // 批次加载店铺列表，每次加载2个 limit = 20
      shopListArr: [], // 店铺列表数据
      touchend: false, // 没有更多数据
      showLoading: true, // 显示家在动画
      showBackStatus: false, // 系那是返回顶部按钮
  ...
  methods: {
    ...
    async loaderMore() {
      ...
      this.hideLoading()
      this.showLoading = [...this.shopListArr,...resJson]
  ...
  watch: {
    // 监听父级传来的restaurantCategoryIds，当前发生变化的时候重新获取参观数据，作用于排序和筛选
