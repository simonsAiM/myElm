<template>
  <section class="cart_module">
    <section v-if="!foods.specifications.length" class="cart_button">
      <transition name="shwoReduce">
        <span @click="removeOutCart(foods.category_id, foods.item_id, foods.sepcfoods[0].food_id, foods.specfoods[0].name, foods.specfoods[0].price, '', foods.specfoods[0].packing_fee, foods.specfoods[0].sku_id, foods.specfoods[0].stock)" v-if="foodNum">
          <svg>
...
export default {
  ...
  methods: {
    ...
    addToCart(category_id, item_id, food_id, name, price, specs, packing_fee, sku_id, stock, event){
      ...
      let elBottom = event.target.getBoundingClientRect().elBottom
